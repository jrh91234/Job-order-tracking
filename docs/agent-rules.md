...
1. Identify the feature/bug area from the user request.
2. Select likely target paths before opening files.
3. Read key files in that area first (entry points, handlers, tests, configs).
4. If confidence is low, ask exactly 1 clarifying question, then continue.
